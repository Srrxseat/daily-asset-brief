...
3. ส่งด้วย **Bash** (curl POST):
   ```bash
   curl -s -o /tmp/line_resp.txt -w "%{http_code}" \
     -X POST https://api.line.me/v2/bot/message/push \
     -H "Content-Type: application/json" \
     -H "Authorization: Bearer ${LINE_CHANNEL_ACCESS_TOKEN}" \
     -d "{\"to\":\"${LINE_TO}\",\"messages\":[{\"type\":\"text\",\"text\":\"<msg>\"}]}"
   ```
   จับ exit code และ HTTP status code ที่ได้จาก `-w "%{http_code}"`

4. ถ้า status ≠ 200 → log `ERROR: LINE API returned {status} — $(cat /tmp/line_resp.txt)` ห้าม retry
